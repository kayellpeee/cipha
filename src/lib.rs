...
mod feistel_tests {
    ...
    fn delta() {
        ...
        println!("heh {:?}", feistel_encrypt("In the election that followed, his widow, Sirimavo
                Bandaranaike, became prime minister on the sympathy
                vote.", 1965200, 158));
    }
    #[bench]
    fn sentence_encryption(b: &mut Bencher) {
        ...
        b.iter(|| feistel_encrypt(plaintext, 1965200, 158));
    }
}
